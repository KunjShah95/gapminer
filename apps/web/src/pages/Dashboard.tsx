...
function ScoreCircle({ score }: { score: number | undefined }) {
  ...
  return (
    <div
      className={`w-14 h-14 rounded-full ${bgColor} flex items-center justify-center relative group`}
    >
      ...
      <span className={`text-sm font-bold skew-x-[-2deg] ${color}`}>
        {score ?? "-"}
      </span>
    </div>
  );
          ...
          <div className="relative group overflow-hidden bg-surface-container-highest rounded-[2.5rem] border border-outline-variant/20">
            ...
            <div className="p-10 flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
              <div className="max-w-xl">
                <span className="bg-primary/20 text-primary px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest mb-4 inline-block">
                  New Version 2.0
                </span>
                <h1 className="text-3xl font-extrabold tracking-tighter mb-4 font-headline">
                  Ready for your next analysis?
                </h1>
                <p className="text-on-surface-variant leading-relaxed font-light mb-6">
                  Our new 5-agent pipeline now supports real-time market
                  indexing. Upload your latest resume to see how you rank
                  against today's top engineering roles.
                </p>
                <Link
                  to="/analyze"
                  className="primary-gradient text-on-primary-fixed px-8 py-3.5 rounded-2xl font-bold inline-flex items-center gap-2 shadow-xl hover:shadow-primary/20 transition-all active:scale-95"
                >
                  <Plus size={20} />
                  Start New Analysis
                </Link>
              ...
              <div className="flex-shrink-0 grid grid-cols-2 gap-3">
                {[
                  ...
                  {
                    label: "Complete",
                    val: analyses.filter((a) => a.status === "complete").length,
                    icon: Zap,
                    color: "text-error",
                  },
                  {
                    label: "Plan",
                    val: user?.plan?.toUpperCase() || "FREE",
                    icon: Award,
                  ...
                  <div
                    key={stat.label}
                    className="glass bg-surface-container p-4 rounded-3xl border border-outline-variant/10 text-center w-32"
                  >
                    <stat.icon
              ...
              <div className="space-y-4">
                {loading ? (
                  ...
                  analyses.map((analysis) => (
                    <div
                      key={analysis.id}
                      className="glass bg-surface-container-high p-6 rounded-[2rem] border border-outline-variant/15 hover:border-primary/30 transition-all group"
                    >
            ...
            <div className="space-y-8">
              ...
              <div className="glass bg-surface-container-high rounded-[2rem] border border-outline-variant/15 p-8 relative overflow-hidden">
                ...
                <div className="text-center">
                  ...
                  <Link
                    to="/profile"
                    className="w-full glass border border-outline-variant/20 py-3 rounded-2xl font-bold text-sm block hover:bg-surface-container-highest transition-all"
                  >
                    Upgrade to Pro
                  </Link>
                </div>
              ...
              <div className="glass bg-surface-container-high rounded-[2rem] border border-outline-variant/15 p-8">
                    ...
                    (analyses[0] as any).top_gaps.map((gap: any, i: number) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-2 h-2 rounded-full ${gap.severity === "critical" ? "bg-error animate-pulse shadow-[0_0_8px_rgba(255,110,132,0.8)]" : "bg-tertiary"}`}
                          ></div>
                          <span className="text-sm font-semibold group-hover:text-primary transition-colors">
                            {gap.skill}
                          </span>
                        </div>
                        <span className="text-[10px] font-bold text-outline opacity-60 uppercase tracking-tighter">
                ...
                {analyses.length > 0 && (
                  <Link
                    ...
                    className="w-full mt-6 text-xs font-bold text-primary hover:underline text-center block"
                  >
                    Explore Details →
                  </Link>
                )}
                ...
                <div className="p-4 rounded-2xl bg-surface-container border border-outline-variant/10 flex items-center gap-4 hover:border-primary/20 transition-all cursor-pointer">
                  ...
                  <div className="overflow-hidden">
                    <div className="text-xs font-bold truncate">
                      Default Analysis Resume
                    </div>
                    <div className="text-[10px] text-outline">
                      Used in {analyses.length} reports
                    </div>
                  </div>
                </div>
              </div>
