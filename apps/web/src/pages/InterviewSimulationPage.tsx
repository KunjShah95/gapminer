...
import { 
  ...
  MessageSquare, BarChart3, Clock, AlertCircle,
...
export default function InterviewSimulationPage() {
  ...
  const [currentQuestion, setCurrentQuestion] = useState("Could you explain how you've used Kubernetes in a production environment to solve scalability issues?")
  ...
  const handleStartSession = () => {
    setIsStarted(true)
    setTranscript(["AI: " + currentQuestion])
  }
  ...
  const toggleRecording = () => {
    ...
    if (!isRecording) {
      // Simulate Whisper speech-to-text
      setTimeout(() => {
        setTranscript(prev => [...prev, "User: In my last project, we used K8s with HPA for a microservices architecture handling 10k requests per second..."])
        setTranscript(prev => [...prev, "AI: That's interesting. How did you handle stateful services in that cluster?"])
        setCurrentQuestion("How did you handle stateful services in that cluster?")
      }, 3000)
    }
  }

  if (showResults) {
    return <InterviewResults scorecard={{ confidence: 8.5, clarity: 9.0, technicalCorrectness: 7.8 }} />
  }
    ...
    <div className="flex flex-col h-full bg-[#050505] text-white font-body overflow-hidden relative">
      ...
      <header className="h-20 flex items-center justify-between px-12 border-b border-white/5 z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-sm font-black tracking-tight skew-x-[-2deg]">Gapminer Live Screening</h1>
            <div className="flex items-center gap-2 mt-0.5">
              <div className="w-1.5 h-1.5 rounded-full bg-success animate-pulse" />
              <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Enterprise Session Active</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <button onClick={handleEndSession} className="px-6 py-2 bg-error/10 border border-error/30 text-error rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-error hover:text-white transition-all">
            End Session
          </button>
        </div>
      </header>
        ...
        <section className="flex-grow flex flex-col gap-8 relative group">
          <div className="flex-grow relative rounded-[3rem] overflow-hidden border border-white/10 bg-[#0a0a0a] shadow-2xl">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center">
              ...
              <h2 className="text-2xl font-black mb-4 tracking-tight">AI Technical Specialist</h2>
              <p className="text-white/40 text-sm max-w-sm leading-relaxed font-medium">
                I'll be conducting your technical screening today. Let's focus on your recent experience with distributed systems.
              </p>
            </div>
          ...
          <div className="h-24 flex items-center justify-center gap-6">
            ...
            <button 
              ...
              className={cn(
                ...
                isRecording ? "bg-error text-white animate-pulse" : "primary-gradient text-white"
              )}
            >
        ...
        <aside className="w-[400px] flex flex-col gap-6 shrink-0 z-10">
          <div className="flex-grow flex flex-col border border-white/5 bg-white/5 rounded-[2.5rem] overflow-hidden backdrop-blur-xl">
            <div className="p-8 border-b border-white/5 flex items-center justify-between">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Session Transcript</h3>
            ...
            <div className="flex-grow overflow-y-auto p-8 space-y-6 custom-scrollbar">
              {!isStarted && (
                <div className="h-full flex flex-col items-center justify-center text-center">
                  <div className="w-16 h-16 rounded-3xl bg-primary/10 flex items-center justify-center text-primary mb-6 animate-pulse">
                    <Shield size={32} />
                  </div>
                  <h4 className="text-lg font-black tracking-tight mb-2">Ready to screen?</h4>
                  <p className="text-xs text-white/40 leading-relaxed max-w-[240px]">
                    This session will test your skills in distributed systems and cloud architecture.
                  </p>
                  <button 
                    onClick={handleStartSession}
                    className="mt-8 px-12 py-3 primary-gradient rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:scale-105 transition-all"
                  >
                    Start Interview
                  </button>
                </div>
              ...
              {transcript.map((line, idx) => (
                ...
                >
                  <span className="font-black text-[9px] uppercase tracking-widest block mb-2 opacity-40">
                    {line.startsWith("AI:") ? "AI Specialist" : "Candidate Response"}
                  </span>
          ...
          <div className="glass bg-primary/10 border border-primary/20 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
            ...
            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary mb-4">Initial Indicators</h4>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-white/60">Clarity</span>
                <span className="text-[11px] font-black text-primary">SCANNING...</span>
              </div>
    ...
    <div className="flex flex-col h-full bg-[#050505] text-white p-12 overflow-hidden items-center justify-center">
       ...
       >
         ...
         <div className="flex items-center gap-6 mb-12">
            ...
            <div>
              <h2 className="text-4xl font-black tracking-tight skew-x-[-2deg]">Session Completed</h2>
              <p className="text-white/40 font-semibold tracking-widest text-xs uppercase mt-2">Enterprise-Grade Performance Scorecard</p>
            </div>
         </div>

         <div className="grid grid-cols-3 gap-8 mb-16">
            {[
              { label: 'Confidence', value: scorecard.confidence, color: 'primary' },
              { label: 'Clarity', value: scorecard.clarity, color: 'tertiary' },
              { label: 'Technical depth', value: scorecard.technicalCorrectness, color: 'success' },
            ].map((stat) => (
              <div key={stat.label} className="bg-white/5 border border-white/10 p-10 rounded-[2.5rem] relative group hover:bg-white/10 transition-all">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 block mb-2">{stat.label}</span>
                <div className="text-5xl font-black tabular-nums">{stat.value}<span className="text-xl text-white/20">/10</span></div>
         ...
         <div className="space-y-8">
            <div className="p-10 border border-primary/30 bg-primary/5 rounded-[2.5rem]">
               <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary mb-6">Gapminer Strategic Feedback</h4>
               <p className="text-lg font-medium leading-relaxed italic text-white/90">
                 "Your explanation of horizontal pod autoscaling was technically precise, though you could strengthen your performance by detailing how you've handled database migrations during cluster rollouts. Your confidence remains high throughout architectural deep-dives."
               </p>
            </div>
