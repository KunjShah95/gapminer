...
router.post('/create-checkout-session', requireUser, async (req, res, next) => {
  try {
    ...
    if (!planId || !['pro', 'teams'].includes(planId)) {
      return res.status(400).json({ error: 'Invalid plan. Must be "pro" or "teams"' });
    }
...
router.post('/create-portal-session', requireUser, async (req, res, next) => {
  try {
    const userId = req.userId;
...
// ─── GET /subscription ─────────────────────────────────────────
router.get('/subscription', requireUser, async (req, res, next) => {
  try {
    const userId = req.userId;
    ...
    return res.json({
      subscription: {
        status: stripeData?.status || userData.subscription_status || 'inactive',
        plan: userData.subscription_plan || userData.plan || 'free',
        currentPeriodEnd: stripeData?.currentPeriodEnd || userData.subscription_current_period_end,
