...
import { 
  ...
  CheckCircle, ChevronRight, Building2, MapPin,
  Briefcase, Calculator, MessageSquare, FileText,
  RefreshCw, Loader2
} from 'lucide-react'
...
export default function NegotiationCompanionPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'tactics' | 'data'>('overview')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/v1/negotiation/strategy', {
        ...
        body: JSON.stringify({
          ...formData,
          currentOffer: formData.currentOffer.base > 0 ? formData.currentOffer : null,
          competingOffers: formData.hasCompetingOffer ? [formData.currentOffer] : [],
        }),
      })
  ...
  return (
    <div className="p-8 lg:p-12 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-black tracking-tighter mb-2">Negotiation Companion</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="glass p-6 rounded-3xl border border-outline-variant/10">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-black uppercase text-outline mb-1 block">Base Salary</label>
                    <input
                      type="number"
                      placeholder="150000"
                      className="w-full bg-surface-container-high border border-outline-variant/20 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-primary"
                      ...
                      onChange={(e) => setFormData({
                        ...formData, 
                        currentOffer: {...formData.currentOffer, base: parseInt(e.target.value) || 0}
                      })}
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase text-outline mb-1 block">Bonus</label>
                    <input
                      type="number"
                      placeholder="20000"
                      className="w-full bg-surface-container-high border border-outline-variant/20 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-primary"
                      ...
                      onChange={(e) => setFormData({
                        ...formData, 
                        currentOffer: {...formData.currentOffer, bonus: parseInt(e.target.value) || 0}
                      })}
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase text-outline mb-1 block">Annual Stock</label>
                    <input
                      type="number"
                      placeholder="50000"
                      className="w-full bg-surface-container-high border border-outline-variant/20 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-primary"
                      ...
                      onChange={(e) => setFormData({
                        ...formData, 
                        currentOffer: {...formData.currentOffer, stock: parseInt(e.target.value) || 0}
                      })}
                    />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase text-outline mb-1 block">Signing Bonus</label>
                    <input
                      type="number"
                      placeholder="25000"
                      className="w-full bg-surface-container-high border border-outline-variant/20 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-primary"
                      ...
                      onChange={(e) => setFormData({
                        ...formData, 
                        currentOffer: {...formData.currentOffer, signing: parseInt(e.target.value) || 0}
                      })}
                    />
                  </div>
                </div>
                ...
                {formData.currentOffer.base > 0 && (
                  <div className="mt-4 p-4 rounded-xl bg-surface-container-highest border border-primary/20">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-black uppercase text-outline">Total Compensation</span>
            ...
            <label className="flex items-center gap-3 p-4 rounded-xl bg-surface-container-high cursor-pointer">
              <input
                ...
                onChange={(e) => setFormData({...formData, hasCompetingOffer: e.target.checked})}
              />
              <span className="text-sm font-medium">I have a competing offer</span>
            </label>
          ...
          {strategy ? (
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-3 gap-4">
                    <div className="glass p-5 rounded-2xl border border-outline-variant/10">
                      <div className="text-[10px] font-black uppercase text-outline mb-2">Opening Ask</div>
                      <div className="text-2xl font-black text-tertiary">
                        {formatCurrency(strategy.openingAnchor)}
                      </div>
                    </div>
                    <div className="glass p-5 rounded-2xl border border-outline-variant/10">
                      <div className="text-[10px] font-black uppercase text-outline mb-2">Target</div>
                      <div className="text-2xl font-black text-primary">
                        {formatCurrency(strategy.targetNumber)}
                      </div>
                    </div>
                    <div className="glass p-5 rounded-2xl border border-outline-variant/10">
                      <div className="text-[10px] font-black uppercase text-outline mb-2">Walk Away</div>
                  ...
                  <div className="glass p-6 rounded-3xl border border-outline-variant/10">
                    <h3 className="text-sm font-black uppercase tracking-widest mb-4">Key Negotiation Levers</h3>
                    <div className="space-y-3">
                      {strategy.keyNegotiationLevers.map((lever, idx) => (
                        <div key={idx} className="flex items-start gap-3 p-3 rounded-xl bg-surface-container-high">
                          <div className={`w-2 h-2 rounded-full mt-1.5 ${
                            lever.priority === 'high' ? 'bg-primary' : 
                            lever.priority === 'medium' ? 'bg-tertiary' : 'bg-outline'
                          }`} />
                          <div>
                            <div className="font-bold text-sm">{lever.lever}</div>
                            <div className="text-xs text-on-surface-variant">{lever.impact}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {strategy.competingOfferStrategy.shouldUse && (
                    <div className="glass p-6 rounded-3xl border border-tertiary/20 bg-tertiary/5">
                      ...
                      <p className="text-sm mb-4">{strategy.competingOfferStrategy.howToPresent}</p>
                      <div className="space-y-2">
                        <div className="text-xs font-black uppercase text-outline">Risks to Consider</div>
              ...
              {activeTab === 'tactics' && (
                <div className="space-y-4">
                  ...
                  {strategy.talkingPoints.map((item, idx) => (
                    <div key={idx} className="glass p-5 rounded-2xl border border-outline-variant/10">
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded ${
                          item.timing === 'early' ? 'bg-error/20 text-error' :
                          item.timing === 'mid' ? 'bg-warning/20 text-warning' :
                    ...
                    <div key={idx} className="glass p-5 rounded-2xl border border-outline-variant/10">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-bold text-sm">{bench.tier.toUpperCase()} Companies</span>
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div>
                          <div className="text-[10px] font-black uppercase text-outline mb-1">Base Range</div>
                          <div className="text-sm font-bold">
                            {formatCurrency(bench.minSalary)} - {formatCurrency(bench.maxSalary)}
                          </div>
                        </div>
                        <div>
                          <div className="text-[10px] font-black uppercase text-outline mb-1">Median Base</div>
                          <div className="text-sm font-bold text-primary">
                            {formatCurrency(bench.medianSalary)}
                          </div>
                        </div>
                        <div>
                          <div className="text-[10px] font-black uppercase text-outline mb-1">Total Comp</div>
                          <div className="text-sm font-bold">
            ...
            <div className="glass p-12 rounded-3xl border border-outline-variant/10 text-center">
              ...
              <p className="text-on-surface-variant text-sm max-w-md mx-auto">
                Enter your target role, location, and offer details to get a personalized negotiation strategy backed by real market data.
              </p>
            </div>
          )}
        </div>
